...
using Content.Shared._Floof.Lewd.Milker;
...
public sealed class MilkerVisualsOverlay : Overlay
{
    ...
    private ISawmill Log => Logger.GetSawmill("milker-visuals");
    private Ticker _logTicker = new(TimeSpan.FromSeconds(3));

    public MilkerVisualsOverlay(IEntityManager entMan)
    {
        _entMan = entMan;
    ...
    // TBH all of this is copied from leash visuals - I have no idea how transforms work so I'm just plugging in numbers and hoping everything works :clueless:
    // If you have any better ideas - implement them.
    protected override void Draw(in OverlayDrawArgs args)
    {
        var worldHandle = args.WorldHandle;
        worldHandle.SetTransform(Vector2.Zero, Angle.Zero);

        var query = _entMan.EntityQueryEnumerator<MilkerComponent>();
        while (query.MoveNext(out var uid, out var comp))
        {
            if (uid is not {Valid: true} source
                || comp.MilkedEntity is not {Valid: true} target
            ...
            var texture = _sprites.Frame0(comp.TubeSprite);
            ...
            var coordsA = _xform.ToMapCoordinates(targetXform.Coordinates, false);
            var coordsB = _xform.ToMapCoordinates(sourceXform.Coordinates, false);
            ...
            ExtractAnchorData(args, (source, sourceXform), comp, out var rotA, out var offsetA, true);
            ExtractAnchorData(args, (target, targetXform), comp, out var rotB, out var offsetB, false);
    ...
    private void ExtractAnchorData(OverlayDrawArgs args, Entity<TransformComponent> leashedEntity, MilkerComponent visualsComp, out Angle rotation, out Vector2 offset, bool entityIsSource)
    {
        rotation = _xform.GetWorldRotation(leashedEntity.Comp);
        offset = (entityIsSource ? visualsComp.TubeOffsetSource : visualsComp.TubeOffsetTarget);
